...
// Jacobi Method
double iterateJ(vector<vector<double>> &V, double deltaV, double p, vector<vector<int>> &P)
{
  auto Vtmp = V;
  double charge = 0.0;
  double dVmax=1e-50;
  ...
  for (int i=1; i<nx-1; i++)
	...
	{
	  for(std::vector<vector<int>>::iterator it = P.begin(); it != P.end(); it++)
	    {
	      if(it[0] == i && it[1] == j)
		{
		  charge = 1.0;
	  ...
	  double Vnew = 0.25*(Vtmp[i+1][j]+Vtmp[i-1][j]+Vtmp[i][j+1]+Vtmp[i][j-1]) + M_PI*charge;;
	  double dV=fabs(Vnew-V[i][j]);
	  dVmax=std::max(dVmax,dV);    // keep track of max change in this sweep
	  if(fabs(V[i][j]) != deltaV/2.0) V[i][j] = Vnew;
	  charge = 0.0;
	}
    }
  return dVmax;
...
TGraph2D* LaplaceLine(int maxIter=100, double eps=0.001, int Npts=100, TCanvas *tc=0, int rate=10, double deltaV=200.0, double spacing=20.0, double width=40.0, double p=1)
{
